...
declare namespace RoactRodux {
	...
	type GetDefaultProps<C> = C extends { defaultProps: infer TDefaults } ? TDefaults : never;

	interface DispatchProp<A extends Rodux.Action = Rodux.AnyAction> {
		dispatch: Rodux.Dispatch<A>;
	}

	type MapStateToProps<TStateProps, TOwnProps, State> = (state: State, props: TOwnProps) => TStateProps;

	type ConnectedComponent<C extends ComponentType<any>, TInjectedProps> = C extends { defaultProps: any }
